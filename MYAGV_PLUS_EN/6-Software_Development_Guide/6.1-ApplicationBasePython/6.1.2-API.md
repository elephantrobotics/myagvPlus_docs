...
- **Function:** Set the underlying board debug state.
- **Parameters:**
...
- **Returns:** `int` (Current debug state bit)
...
- **Function:** Switch the control mode of a specified motor.
- **Parameters:**
  - `motor_id` (int): 1~4
  - `control_mode`: (Enum) E.g., `Control_Type.VEL`, etc.

### change_motor_param(motor_id, rid, value)

- **Function:** Temporarily modify the DM motor's internal advanced control register parameter.
- **Parameters:**
  - `motor_id` (int): 1~4
  - `rid`: Target parameter ID (refer to DM manual, e.g., 0 for UV_Value)
  - `value`: Parameter value to be written

### read_motor_param(motor_id, rid)

- **Function:** Read a specified advanced parameter from the DM motor.

### save_motor_param(motor_id)

- **Function:** Permanently save the modified DM motor parameters into Flash storage.
- **Parameters:** `motor_id` (int): 1~4
