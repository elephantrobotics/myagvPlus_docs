...
Due to the limitations of myagv Plus express delivery safety, the battery power is generally very low when users first get myagv Plus. At this time, the power is not enough to start myagv Plus, so you need to charge myagv Plus. Take out the 12.6V power adapter from the accessory box and connect the DC power supply to charge myagv Plus.
...
3. If the system fails to start successfully, the following common issues may be the cause. If myagv still cannot enter the system, please contact technical support personnel for assistance in troubleshooting:
   ...
   - The lithium battery has entered battery protection mode. The voltage is too low to power the myagv Plus. Please use a 12.6V power adapter to [charge](#1-external-cable-connection) the myagv Plus. 
   ...
   - The system files are damaged. You can try [replacing them with the latest image system.](../9-FilesDownload/9.4-SystemInformation/9.4.2-Image_Burning.md)
...
Find the 'Test' window at the bottom left corner, select the hardware you want to test. The hardware that can be tested on myAGV Plus includes the following: motors, LEDs, 2D cameras.

**One important note is that when testing the motors, they will rotate at high speed. It is recommended to place an object under the myagv Plus to elevate it, allowing the four wheels to be suspended before conducting the test.**
