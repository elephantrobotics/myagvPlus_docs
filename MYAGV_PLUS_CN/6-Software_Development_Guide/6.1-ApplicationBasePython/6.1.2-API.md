...
> **注意：**
>
> 1. 平移移动指令的速度参数 `speed` 的有效范围均为 **0.01 ~ 1.60 m/s**。传入越界参数将直接引发警告并返回。
...
- **参数:**
  - `motor_id` (int): 1~4
  - `control_mode`: (枚举值) 例如 `Control_Type.VEL` 等

### change_motor_param(motor_id, rid, value)

- **功能:** 临时修改 DM 电机高级内部控制寄存器参数
- **参数:**
  - `motor_id` (int): 1~4
  - `rid`: 目标参数 ID (详见 DM 手册，例如 0为UV_Value)
  - `value`: 需要写入的参数值
...
- **功能:** 将修改的 DM 电机参数永久固化保存到 Flash 中
- **参数:** `motor_id` (int): 1~4
...
- **参数:** 
  - `state` (int): 0-串口通讯 (默认), 1-Socket 通讯, 2-蓝牙通讯
  - `force` (bool): 是否强制覆盖锁定状态
- **返回值:** `int` (1: 成功, -1: 失败)
