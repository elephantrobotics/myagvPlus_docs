...
- **返回值:** `list[int]` `[电池状态, 陀螺仪状态, 电量水平]`
  ...
  - **注释:** 充电状态下，小于19.8V为2-低电量 19.8V-20.3V为1-警告，大于20.3为0-正常。
  - **注释:** 不充电状态下，小于19.2V为2-低电量 19.2V-19.7V为1-警告，大于19.7为0-正常。
...
- **参数:**
  - `pin` (int): 引脚编号 (2 或 5)
  - `state` (int): 0 - 低电平 (吸泵工作开启), 1 - 高电平 (吸泵关闭)
- **返回值:** `int` (1: 成功, -1: 失败)
